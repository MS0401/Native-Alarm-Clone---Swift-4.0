...
class Scheduler : AlarmSchedulerDelegate
{
    ...
    func setupNotificationSettings() -> UIUserNotificationSettings {
        ...
        alarmCategory.identifier = "AlarmClock"
    ...
    private func correctDate(_ date: Date, onWeekdaysForNotify weekdays:[Int]) -> [Date]
    {
        ...
        //repeat
            ...
            //current alarm save
            if date < now {
                print("Because date is before than current date removed from alarm list.")
            }
            else { //later
                var same: Bool = false
                for wd in weekdays {
                    if GetScheduledEEEE(date: date) == wd {
                        same = true
                        break
                    }
                    same = false
...
//        AlarmNotification.repeatInterval = NSCalendar.Unit.minute
    ...
    func setNotificationForSnooze(snoozeMinute: Int, soundName: String, mediaID: String, index: Int) {
        let calendar = Calendar(identifier: Calendar.Identifier.gregorian)
        let now = Date()
        ...
        setNotificationWithDate(snoozeTime, onWeekdaysForNotify: [Int](), snoozeEnabled: true, onSnooze:true, soundName: soundName, mediaID: mediaID, index: index)
    }
    ...
    func reSchedule() {
        ...
        for i in 0..<alarmModel.count{
            ...
            if alarm.enabled {
                setNotificationWithDate(alarm.date as Date, onWeekdaysForNotify: alarm.repeatWeekdays, snoozeEnabled: alarm.snoozeEnabled, onSnooze: false, soundName: alarm.mediaLabel, mediaID: alarm.mediaID, index: i)
            }
        }
    }
