...
class MainAlarmViewController: UITableViewController{
    ...
    override func viewDidLoad() {
        ...
        self.navigationController?.navigationBar.barTintColor = UIColor.init(netHex: 0x191919)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let back = UIBarButtonItem(title: "Back", style: .plain, target: self, action: #selector(MainAlarmViewController.GoBack))
        if alarmModel.count != 0 {
            self.navigationItem.setLeftBarButtonItems([editButtonItem, back], animated: true)
        }
        else {
    ...
    @IBAction func switchTapped(_ sender: UISwitch) {
        ...
        if sender.isOn {
            print("switch on")
            alarmScheduler.setNotificationWithDate(alarmModel.alarms[index].date, onWeekdaysForNotify: alarmModel.alarms[index].repeatWeekdays, snoozeEnabled: alarmModel.alarms[index].snoozeEnabled, onSnooze: false, soundName: alarmModel.alarms[index].mediaLabel, mediaID: alarmModel.alarms[index].mediaID, index: index)
            tableView.reloadData()
        }
        else {
            print("switch off")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == Id.addSegueIdentifier {
            ...
            addEditController.segueInfo = SegueInfo(curCellIndex: alarmModel.count, isEditMode: false, label: "Alarm", mediaLabel: "Classic Alarm", mediaID: "", repeatWeekdays: [], enabled: false, snoozeEnabled: true)
        }
